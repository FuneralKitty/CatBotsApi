...
from src.data_fullfilling import cat_colors_create_data, fullfill_cat_options, add_info_db, get_parsed_data
from flask import Flask, request, jsonify, Response
from flask_limiter import Limiter
from flask_limiter.util import get_remote_address


pool = ConnectionPool(conninfo=f"dbname={DB_CONFIG['dbname']} user={DB_CONFIG['user']} password={DB_CONFIG['password']} host={DB_CONFIG['host']} port={DB_CONFIG['port']}")
...
def data_parser() -> tuple[Response, int]:
    ...
    result: Tuple[Dict[str, str], int] = get_parsed_data(pool, attribute, order, offset, limit)
